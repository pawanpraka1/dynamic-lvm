...
The diagram below depicts the mapping to the host disks, the LVM stack on top of the disks and the kubernetes persistent volumes to be consumed by the workload. LVM LocalPV CSI Controller upon creation of the Persistent Volume Claim, creates a LVMVolume CR, which emits an event for LVM LocalPV CSI Node Plugin to create the LV(logical volume). When workloads are scheduled the LVM LocalPV CSI Node Plugin makes this zvol/dataset available via a mount point on the host.
